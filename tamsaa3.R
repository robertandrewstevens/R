...
function(ctime, time)
{
  ...
  #
  #ln.x <- log(time)
  n <- length(time)	#beta.bias <- n/(n * log(ctime) - sum(ln.x))
  #lamb.bias <- n/ctime^beta.bias
  #end.bias <- lamb.bias * beta.bias * ctime^(beta.bias -
  #	1)
  ...
  #end.unbi <- lamb.unbi * beta.unbi * ctime^(beta.unbi -
  #	1)
  ...
  #	)	#
  plot(time, fail, type = "p", pch = "*", xlab = "Cumulative Test Time", ylab = "Cumulative Failures", main = 
  ...
  plot(time, 1000 * fail.rate, type = "p", pch = "*", ylim = c(0, max(1000 * fail.rate)), xlab = "Cumulative Test Time", ylab = "IPTV", 
       main = "IPTV versus Test Time")
}
