# http://www.public.iastate.edu/~hofmann/looking-at-data/rcode/airline%20(Autosaved).R

# don't worry, if this fails - that is only necessary for DB access
# install.packages("DBI", type="source")
# install.packages("RMySQL", type="source")
# for help with the installation see  http://biostat.mc.vanderbilt.edu/wiki/Main/RMySQL
...
theme_nothing <- opts(axis.title.x=theme_blank(), # remove x and y label 
                      ...
                      axis.ticks=theme_blank(), # donâ€™t show ticks
                      axis.text.y = theme_blank(), # no tick marks
                      axis.text.x = theme_blank(),
                      
                      panel.grid.minor=theme_blank(), # donâ€™t show any gridlines 
...
source("http://www.public.iastate.edu/~hofmann/looking-at-data/rcode/ggooglemaps.R")

# get satellite image for the affected area
# if this doesn't work, read data usa.csv 
