...
p2 <- p1 + 
      theme_bw() + 
      #scale_y_continuous(labels = dgrfmt, limits = c(0, 100), expand = c(0, 0) ) + 
      labs(title = expression("Average Monthly Temperatures (" * degree * "F)"), x = NULL, y = NULL) 
      #theme(panel.grid.major = none, panel.grid.minor = none, legend.position = "none", panel.background = none, panel.border = none, axis.line = element_line(colour = "grey50"))
...
data_table <- ggplot(dfm, aes(x = month, y = factor(City), label = format(value, nsmall = 1), colour = City)) + 
              ...
              #theme(panel.grid.major = none, legend.position = "none", panel.border = none, axis.text.x = none, axis.ticks = none, plot.margin = unit(c(-0.5, 1, 0, 0.5), "lines")) + 
              labs(x = NULL, y = NULL)
