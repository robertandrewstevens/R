...
latex(anova(f1), file='', label='titanic-anova3', size='small')
...
## @knitr anova2
latex(anova(f),file='',label='titanic-anova2',size='small')#(*\ref{titanic-anova2}*)
...
m <- lrm(is.na(age) ~ sex * pclass + survived + sibsp + parch,
         data=t3)
print(m, latex=TRUE, needspace='3.5in')


## @knitr anova.na
latex(anova(m), file='', label='titanic-anova.na')# Table (*\ref{titanic-anova.na}*)
...
## @knitr anova.si
latex(anova(f.si), file='', label='titanic-anova.si')   # Table (*\ref{titanic-anova.si}*)
